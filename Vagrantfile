...
Vagrant.configure("2") do |config|
  ...
  config.vm.define NGINX_BOX_NAME do |subconfig|
    ...
    subconfig.vm.hostname = "#{NGINX_BOX_NAME}.cyberpunk"
  ...
  (1..NODE_COUNT).each do |i|
    config.vm.define "node#{i}" do |subconfig|
      subconfig.vm.box = BOX_IMAGE
      subconfig.vm.hostname = "node#{i}.cyberpunk"
  ...
  (1..DB_NODE_COUNT).each do |j|
    config.vm.define "#{DB_BOX_NAME}#{j}" do |subconfig|
      subconfig.vm.box = BOX_IMAGE
      subconfig.vm.hostname ="#{DB_BOX_NAME}#{j}.cyberpunk"
