...
describe('Users', () => {
  it('sanitizeDbUser should handle undefined', () => {
    expect(users.sanitizeDbUser()).to.deep.equal({ id: '', username: '', display: '', roles: '', password: '' })
  });
  it('sanitizeDbUser should handle random data', () => {
    expect(users.sanitizeDbUser({ random: 'ermgh', geerr: 'ddg' })).to.deep.equal({
      id: '',
      username: '',
      display: '',
      roles: '',
  ...
  it('populateUser with missing', async () => {
    const db = {
      queryFunction: (table, username) => [{ get_player_data: null }]
    };
    try {
      await users.populateUser(undefined, db);
    }
    catch (e) {
      expect(e.toString()).to.equal('Error: No record of user: skiddle');
    }
  });
  it('populateUser with correct data', async () => {
    const db = {
      queryFunction: (table, username) => [{ get_player_data: [{ id: 1, username: 2, roles: 3, password: 4 }] }]
    };
    const result = await users.populateUser(undefined, db);
    expect(result).to.deep.equal({ id: 1, username: 2, display: '', roles: 3, password: '' })
  });
  ...
  it('authenticate with missing db', async () => {
    try {
      ...
      expect(e.toString()).to.equal('Error: Missing database');
    }
